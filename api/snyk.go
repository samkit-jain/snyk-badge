...
func badgeHandler(w http.ResponseWriter, r *http.Request, username string, repo string) {
	...
	if err != nil {
		fmt.Println("Errored when sending request to the Snyk server")
	...
	if resp.StatusCode != 200 {
		fmt.Println("Did not receive a 200 OK response from the Snyk server")
		writeBadge(w, badgeUrl)
		return
	}
...
func writeBadge(w http.ResponseWriter, badgeUrl string) {
	...
	// Likely network issues
	if err != nil {
		fmt.Println("Errored when sending request to the Shields server")
	...
	if resp.StatusCode != 200 {
		fmt.Println("Did not receive a 200 OK response from the Shields server")
		fmt.Fprintf(w, badgeUrl)
		return
	}
